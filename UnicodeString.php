...
/**
 * UnicodeString: Immutable Unicode string with comprehensive UTF-8 support.
 * 
 * Provides extensive Unicode string manipulation capabilities including
 * proper character counting, case conversion, normalization, and
 * multi-byte safe operations for international text processing.
 * 
 * @package Flaphl\Element\String
 * @author Jade Phyressi <jade@flaphl.com>
 */
class UnicodeString implements Stringable
{
    /**
    ...
    /**
     * Constructor to create a new UnicodeString.
     *
    ...
    /**
     * Create a UnicodeString from a regular string.
     *
     * @param string $string The string to wrap.
     * @param string $encoding The source encoding.
     * @return self A new UnicodeString instance.
     */
    public static function of(string $string, string $encoding = 'UTF-8'): self
    {
        return new self($string, $encoding);
    }

    /**
     * Create an empty UnicodeString.
     *
     * @return self A new empty UnicodeString instance.
     */
     ...
     * @return self A new UnicodeString with the substring.
     ...
     * @return self A new UnicodeString with the concatenated result.
     */
    public function concat(string|AbstractUnicodeString $other): self
    {
        $otherString = $other instanceof AbstractUnicodeString ? $other->toString() : $other;
        return new self($this->string . $otherString);
    }
    ...
    {
        $needleString = $needle instanceof self ? $needle->string : $needle;
    ...
    {
        $prefixString = $prefix instanceof self ? $prefix->string : $prefix;
    ...
    {
        $suffixString = $suffix instanceof self ? $suffix->string : $suffix;
     ...
     *
     * @return self A new UnicodeString in lowercase.
     */
     ...
     *
     * @return self A new UnicodeString in uppercase.
     */
     ...
     *
     * @return self A new UnicodeString in title case.
     */
     ...
     * @return self A new trimmed UnicodeString.
     */
     ...
     *
     * @param string $characters Characters to trim.
     * @return self A new left-trimmed UnicodeString.
     */
     ...
     *
     * @param string $characters Characters to trim.
     * @return self A new right-trimmed UnicodeString.
     */
     ...
     * @return self A new UnicodeString with replacements made.
     */
    public function replace(string|self $search, string|self $replace, bool $caseSensitive = true): self
    {
        $searchString = $search instanceof self ? $search->string : $search;
        $replaceString = $replace instanceof self ? $replace->string : $replace;
     ...
     * @return array An array of UnicodeString parts.
     */
    public function split(string|self $delimiter, int $limit = 0): array
    {
        $delimiterString = $delimiter instanceof self ? $delimiter->string : $delimiter;
        ...
        return array_map(fn($part) => new self($part), $parts);
    }

    /**
     * Reverse the string.
     *
     * @return self A new reversed UnicodeString.
     */
    public function reverse(): self
    {
        ...
        return new self(implode('', array_reverse($characters)));
    }
     ...
     * @return self A new UnicodeString with the repeated content.
     ...
     * @return self A new padded UnicodeString.
     */
    public function pad(int $length, string $padString = ' ', int $padType = STR_PAD_RIGHT): self
    {
        ...
        switch ($padType) {
            case STR_PAD_LEFT:
                return new self($padStr . $this->string);
            case STR_PAD_BOTH:
                $leftPad = str_repeat($padString, floor($padLength / 2));
                $rightPad = str_repeat($padString, ceil($padLength / 2));
                return new self($leftPad . $this->string . $rightPad);
            default:
                return new self($this->string . $padStr);
        }
    }
    ...
    {
        $otherString = $other instanceof self ? $other->string : $other;
     ...
     * @return self A new normalized UnicodeString.
